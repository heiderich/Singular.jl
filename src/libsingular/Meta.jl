    ...
    for i in funcs
        if i[1] == "g"
            ...
            if haskey(output_maniuplator_funcs,name) && haskey(output_maniuplator_funcs[name],symb)
                push!(func_calls, :($symb(args...) = $(output_maniuplator_funcs[name][symb])(libSingular.low_level_caller($(name_string),$func_name,args...)) ))
                push!(func_calls, :($symb(ring::PolyRing,args...) = $(output_maniuplator_funcs[name][symb])(libSingular.low_level_caller_rng($(name_string),$func_name,ring,args...)) ))
            else
